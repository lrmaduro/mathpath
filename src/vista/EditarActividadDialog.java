...
public class EditarActividadDialog extends JDialog {
    ...
    public EditarActividadDialog(JFrame parent, List<Ejercicio> ejercicios, List<Tema> temas, Actividad actividad) {
        ...
        JLabel lblSub = new JLabel("Modifica los detalles de la actividad.");
        ...
        gbc.gridy = 2;
        gbc.weightx = 0;
        panelDatos.add(crearLabel("Fecha Límite:"), gbc);

        gbc.gridx = 1;
        spinnerFecha = new javax.swing.JSpinner(new javax.swing.SpinnerDateModel());
        javax.swing.JSpinner.DateEditor de = new javax.swing.JSpinner.DateEditor(spinnerFecha, "dd/MM/yyyy HH:mm");
        spinnerFecha.setEditor(de);

        if (actividad.getFechaLimite() != null) {
            java.util.Date date = java.util.Date
                    .from(actividad.getFechaLimite().atZone(java.time.ZoneId.systemDefault()).toInstant());
            spinnerFecha.setValue(date);
        }

        panelDatos.add(spinnerFecha, gbc);
        ...
        if (ejercicios.isEmpty()) {
            ...
            for (Ejercicio ej : ejercicios) {
                String etiqueta = String.format("<html><font color='#2980B9'><b>%s</b></font>: %s</html>", ej.getId(),
                        ej.getPregunta());
        ...
        btnGuardar.addActionListener(e -> {
            if (txtNombre.getText().trim().isEmpty()) {
                JOptionPane.showMessageDialog(this, "Por favor, ingresa un nombre para la actividad.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            if (cmbTema.getSelectedItem() == null) {
                JOptionPane.showMessageDialog(this, "Por favor, selecciona un tema para la actividad.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            if (spinnerFecha.getValue() == null) {
                JOptionPane.showMessageDialog(this, "Por favor, selecciona una fecha límite válida para la actividad.",
    ...
    public Actividad getActividadEditada() {
        ...
        java.time.LocalDateTime fechaLimite = date.toInstant()
                .atZone(java.time.ZoneId.systemDefault())
                .toLocalDateTime();
